						...
						<table class="table table-bordered p-0 m-0" id="projectDtlTable">
							<thead class="table-light">
								<tr>
									...
									<th class="text-center align-middle">Image Data & Purpose</th>
									<th class="text-center align-middle">Surface Treatment(기구)<br>/Maker Spec(전장,광학)</th>
									<th class="text-center align-middle">SPEC</th>
	...
	let projectDtlTable = $('#projectDtlTable').DataTable({
        ...
        ajax : {
			...
			data : {
				...
				inputGubun		: function() { return '003';},
		...
		columns : [
			...
			{ data: 'purpose', className : 'text-center align-middle', name: 'purpose', // 세부규격
				render : function(data, type, row, meta) {
					return '<div style="white-space:pre-line;min-width:250px;">'+data+'</div>';
				}
			},
			{ data: 'surfaceTreatment', className : 'text-left align-middle', name: 'surfaceTreatment', // 규격
				render : function(data, type, row, meta) {
					return '<div style="white-space:pre-line;min-width:500px;">'+data+'</div>';
				}
			},
			{ data: 'partDc', className : 'text-left align-middle', name: 'partDc', // 가공처
				render : function(data, type, row, meta) {
					return '<div style="white-space:pre-line;min-width:500px;">'+data+'</div>';
				}
			},
			{ data: 'maker', className : 'text-center align-middle', name: 'maker', // 가공처
				render : function(data, type, row, meta) {
					return '<div style="white-space:pre-line;min-width:150px;">'+data+'</div>';
				}
			},
				...
				$(tr).find('td').each(function(index_td, td) {
					if($(td).find('input').length == 0 && $(td).find('select').length == 0) {
						if(index_td == projectDtlTable.column('positionGubun:name').index()) {
							...
							let value = $(td).text();
							$(td).html('<input type="text" style="min-width:200px;" class="form-control" value="'+value+'">');
						} else if(index_td == projectDtlTable.column('purpose:name').index()) {
							let value = $(td).text();
							let height = $(td).height()+5;
							$(td).html('<textarea class="form-control" style="min-width:250px; resize: none; min-height: '+height+'px;" >'+value+'</textarea>');
						} else if(index_td == projectDtlTable.column('surfaceTreatment:name').index()) {
							let value = $(td).text();
							let height = $(td).height()+5;
							$(td).html('<textarea class="form-control" style="min-width:500px; resize: none; min-height: '+height+'px;" >'+value+'</textarea>');
							...
							let value = $(td).text();
							let height = $(td).height()+5;
							$(td).html('<textarea class="form-control" style="min-width:150px; resize: none; min-height: '+height+'px;" >'+value+'</textarea>');
							...
							let value = $(td).text();
							/*
							let data = parseInt(projectDtlTable.row(tr).data().needQty) * parseInt($('#ordQty').val()) - parseInt(projectDtlTable.row(tr).data().orderQty);
							$(td).html('<input type="text" style="min-width:100px;" onkeyup="numberFormat(this, \'int\')" class="form-control text-end" value="'+data+'">');
							*/
					...
					if(status=='001') {		//수정
						$('#projectDtlTable tbody').find('tr').each(function(index, item) {
							if($(this).find('input[name=dtlCheckBox]').is(':checked')) {
								...
								let purpose = $(td).eq(projectDtlTable.column('purpose:name').index()).find('textarea').val();
								let surfaceTreatment = $(td).eq(projectDtlTable.column('surfaceTreatment:name').index()).find('textarea').val();
								let partDc = $(td).eq(projectDtlTable.column('partDc:name').index()).find('textarea').val();
								let maker = $(td).eq(projectDtlTable.column('maker:name').index()).find('textarea').val();								
	...
	$('#excelUploadFile').change( function() {
		...
		if (str.substring(str.lastIndexOf(".") + 1) == "xls"
			...
			$.ajax({
				...
				data : formData,
				url : '<c:url value="/rm/projectBomElectronicsFileload"/>',
				processData : false,
				contentType : false,
				cache : false,
				beforeSend : function(){
