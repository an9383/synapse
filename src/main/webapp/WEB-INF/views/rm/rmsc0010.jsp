...
<!-- 						<label class="form-label d-flex align-items-center header-label m-0 me-1 h-100"> -->
				...
				<table class="table table-bordered p-0 m-0" id="bizOrderAdmTable">
					<thead class="table-light">
						<tr>
							...
							<th class="text-center align-middle">시작일자</th>
			...
			<div class="row">
				<div style="width: 100%;">
					<div class="row" style="padding: 5px;">
						<table class="table table-bordered p-0 m-0" id="rightHeaderTable">
							<colgroup>
								<col width="11%">
								<col width="22%">
								<col width="11%">
						...
						<table class="table table-bordered p-0 m-0" id="projectDtlTable">
							<thead class="table-light">
								<tr>
									...
									<th class="text-center align-middle">조달구분</th>
									<th class="text-center align-middle">Part No/Code</th>
									<th class="text-center align-middle">Part Name/품명</th>
									<th class="text-center align-middle">Material/재질</th>
									<th class="text-center align-middle">Surface Treatment</th>
									...
									<th class="text-center align-middle">선점재고수량</th>
...
<!-- 						<label class="form-label d-flex align-items-center header-label m-0 me-1 h-100">projectNo.</label> -->
...
// 	$('#bizOrderAdmTable thead th').resizable({
//     handles: "e", // 크기 조절 핸들의 위치를 오른쪽으로 제한
//     maxWidth: 500,
//     minWidth: 0
// });
</script>
...
// 				prjCd		: function() { return $('#projectSearch').val();},
// 			},
// 		},
//         rowId: 'itemCd',
// 		columns : [
...
// 	//
	...
	let bizOrderAdmTable = $('#bizOrderAdmTable').DataTable({
        ...
        rowId: 'idx',
		columns : [
			{ data: 'projectCd', className : 'text-center  align-middle'}, //프로젝트번호
			{ data: 'startDate', className : 'text-center  align-middle', //식자일자
				render : function(data, type, row, meta) {
					if(data != null){
			...
			{ data: 'targetDate', className : 'text-center  align-middle', //목표일자
				render : function(data, type, row, meta) {
					if(data != null){
						return '<div style="white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">'+moment(data).format('YYYY-MM-DD')+'</div>';
					} else {
		...
		drawCallback: function() {
			...
			$('#'+table_id+'_wrapper').find('.dataTables_scrollBody').css('height',(htmlHeight - theadHeight - 85)+'px');
	...
	$('#bizOrderAdmTable tbody').on('click','tr', function() {
		...
		$('#prjCd').val(bizOrderAdmTable.row(this).data().projectCd);
		$('#ordQty').val(addCommas(parseInt(bizOrderAdmTable.row(this).data().prjQty)));
		...
		status = '001';
		bizOrdProjectIdx = bizOrderAdmTable.row(this).data().idx;
		projectAdmSel().then( () => {
	...
	async function projectAdmSel() {
		return new Promise( (resolve) => {
			$.ajax({
	            ...
	            //async: false,
	            data: {
	                'bizOrdProjectIdx'	:	bizOrdProjectIdx,
	                'inputGubun'	:	'001',
	            },
	            beforeSend: function() {
	            	$('#my-spinner').show();
	...
	let projectDtlTable = $('#projectDtlTable').DataTable({
        ...
        ajax : {
			...
			type : 'GET',
			data : {
				bizOrdProjectIdx	: function() { return bizOrdProjectIdx;},
		...
		columns : [
			{ className : 'text-center align-middle', name : "dtlCheckBox", //체크박스
				render: function(data, type, row, meta) {
					if(row['idx']!=null) {
						return '<input class="form-check-input" type="checkbox" name="dtlCheckBox" style="margin: 0px; width: 25px; height: 25px;" onkeydown="cellFocusChange(\'projectDtlTable\', '+meta.row+', 0)">';
					} else {
						return '<input class="form-check-input" type="checkbox" name="dtlCheckBox" style="margin: 0px; width: 25px; height: 25px;" onkeydown="cellFocusChange(\'projectDtlTable\', '+meta.row+', 0)">';
					}
				}
			...
			{ data: 'orderGubun', className : 'text-center align-middle', name: 'orderGubun', //조달구분
				render: function(data, type, row, meta) {
					if(data!=null) {
						if(data=='001') {
							return '<div style="min-width:100px; white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">가공</div>';
						} else {
							return '<div style="min-width:100px; white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">구매</div>';
						}
					} else {
						return '<div style="white-space: nowrap; text-overflow: ellipsis; overflow: hidden;"></div>';
					}
			...
			{ data: 'material', className : 'text-center align-middle', name: 'material', //재질
				render : function(data, type, row, meta) {
					return '<div style="min-width:150px; white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">'+data+'</div>';
				}
			},
			{ data: 'surfaceTreatment', className : 'text-center align-middle', name: 'surfaceTreatment', // 표면처리
				render : function(data, type, row, meta) {
					return '<div style="min-width:150px; white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">'+data+'</div>';
				}
			},
			...
			{ className : 'text-end align-middle', name: 'preemptiveQty', // 선점재고수량
				render : function(data, type, row, meta) {
					return '<div style="min-width:100px; white-space: nowrap; text-overflow: ellipsis; overflow: hidden;"></div>';
				}
			},
		...
		drawCallback: function() {
			...
			let rightHeaderHeight = parseFloat($('#rightHeader').css('height')); //상단 버튼
			let rightHeaderTableHeight = parseFloat($('#rightHeaderTable').css('height')); //상단 테이블 높이
			let theadHeight = parseFloat($('#projectDtlTable_wrapper').find('.dataTables_scrollHead').css('height')); ///필터
			let moduleTabsHeight = parseFloat($('#moduleTabs').css('height')); //만든탭높이
			$('#projectDtlTable_wrapper').find('.dataTables_scrollBody').css('height',(middlecolT_height - rightHeaderHeight - rightHeaderTableHeight - theadHeight - moduleTabsHeight - 54)+'px');
				...
				$(tr).find('td').each(function(index_td, td) {
					if($(td).find('input').length == 0 && $(td).find('select').length == 0) {
						if(index_td == projectDtlTable.column('positionGubun:name').index()) {
							let value = projectDtlTable.row(tr).data().positionGubun;
							let html = '';
							html += '<select style="min-width:150px;" class="form-select" onkeydown="cellFocusChange(\'projectDtlTable\', '+index_tr+', '+index_td+')">';
							if(value=="TOP") {
								html +='<option value="001" selected>TOP</option>';
								html +='<option value="002">BOTTOM</option>';
							} else {
								html +='<option value="001">TOP</option>';
								html +='<option value="002" selected>BOTTOM</option>';
							}
							html += '</select>';
							$(td).html(html);
						} else if(index_td == projectDtlTable.column('purchaseGubun:name').index()) {
							let value = $(td).text();
							$(td).html('<input type="text" style="min-width:150px;" class="form-control" value="'+value+'" onkeydown="cellFocusChange(\'projectDtlTable\', '+index_tr+', '+index_td+')">');
						} else if(index_td == projectDtlTable.column('orderGubun:name').index()) {
							let value = projectDtlTable.row(tr).data().orderGubun;
							let html = '';
							html += '<select style="min-width:100px;" class="form-select" onkeydown="cellFocusChange(\'projectDtlTable\', '+index_tr+', '+index_td+')">';
							if(value=="001") {
							...
							$(td).html('<input type="text" style="min-width:200px;" class="form-control" value="'+value+'" onkeydown="cellFocusChange(\'projectDtlTable\', '+index_tr+', '+index_td+')">');
						} else if(index_td == projectDtlTable.column('partGubun:name').index()) {
							let value = $(td).text();
							$(td).html('<input type="text" style="min-width:80px;" class="form-control" value="'+value+'" onkeydown="cellFocusChange(\'projectDtlTable\', '+index_tr+', '+index_td+')">');
							...
							let value = $(td).text();
							$(td).html('<input type="text" style="min-width:130px;" class="form-control" value="'+value+'" onkeydown="cellFocusChange(\'projectDtlTable\', '+index_tr+', '+index_td+')">');
						} else if(index_td == projectDtlTable.column('needQty:name').index()) {
							let value = $(td).text();
							$(td).html('<input type="text" style="min-width:100px;" onkeyup="numberFormat(this, \'int\')" class="form-control text-end" value="'+value+'" onkeydown="cellFocusChange(\'projectDtlTable\', '+index_tr+', '+index_td+')">');
							...
							$(td).html('<input type="date" style="min-width:130px;" max="9999-12-31" name="orderDate" class="form-control" value="'+moment().format('YYYY-MM-DD')+'" onkeydown="cellFocusChange(\'projectDtlTable\', '+index_tr+', '+index_td+')">');
						} else if(index_td == projectDtlTable.column('orderQty:name').index()) {
							let value = $(td).text();
							$(td).html('<input type="text" name="orderQty" style="min-width:100px;" onkeyup="numberFormat(this, \'int\')" class="form-control text-end" value="'+value+'" onkeydown="cellFocusChange(\'projectDtlTable\', '+index_tr+', '+index_td+')">');
						} else if(index_td == projectDtlTable.column('preemptiveQty:name').index()) {
							let value = $(td).text();
							$(td).html('<input type="text" name="preemptiveQty" style="min-width:100px;" onkeyup="numberFormat(this, \'int\')" class="form-control text-end" value="'+value+'" onkeydown="cellFocusChange(\'projectDtlTable\', '+index_tr+', '+index_td+')">');
							...
							$(td).html('<textarea class="form-control" style="min-width:200px; resize: none; min-height: '+height+'px;" onkeydown="cellFocusChange(\'projectDtlTable\', '+index_tr+', '+index_td+')">'+value+'</textarea>');
						}
					}
				})
			}
					...
					if(status=='001') {		//수정
						$('#projectDtlTable tbody').find('tr').each(function(index, item) {
							if($(this).find('input[name=dtlCheckBox]').is(':checked')) {
								...
								let orderGubun = $(td).eq(projectDtlTable.column('orderGubun:name').index()).find('select').val();
								...
								let material = $(td).eq(projectDtlTable.column('material:name').index()).find('input').val();
								...
								let preemptiveQty = $(td).eq(projectDtlTable.column('preemptiveQty:name').index()).find('input').val();
...
// 								}
								...
								obj.preemptiveQty = preemptiveQty.replace(/,/g, '');
			...
			setTimeout(function() {
				$('#projectDtlTable tbody').find('tr').each(function(index, item) {
					if($(this).find('input[name=dtlCheckBox]').is(':checked')) {
						...
						let preemptiveQty = $(td).eq(projectDtlTable.column('preemptiveQty:name').index()).find('input').val();
						if(preemptiveQty == null || preemptiveQty == ''){
							preemptiveQty = 0;
						}
						//let inventoryQty = $(td).eq(projectDtlTable.column('inventoryQty:name').index()).find('input').val();
						let remark = $(td).eq(projectDtlTable.column('remark:name').index()).find('textarea').val();
...
// 			dealCorpFileTable.ajax.reload(function() {});
	...
	$('#excelUploadFile').change( function() {
		...
		if (str.substring(str.lastIndexOf(".") + 1) == "xls"
			...
			$.ajax({
				...
				data : formData,
				url : '<c:url value="/rm/projectBomMachineFileload"/>',
				processData : false,
				contentType : false,
				cache : false,
				beforeSend : function(){
	...
	$('#btnProjectReqst').on('click',function() { // 구매요청 버튼 클릭
		...
		projectCd = bizOrderAdmTable.row('.selected').data().projectCd;
	...
	let projectReqstTable = $('#projectReqstTable').DataTable({
		...
		columns : [
			...
			{ data: 'preemptiveQty', className : 'text-end align-middle', name: 'preemptiveQty', // 선점재고수량
				render : function(data, type, row, meta) {
					return '<div style="min-width:100px; white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">'+addCommas(parseInt(data))+'</div>';
				}
			},
	...
	$('#btnProjectReqstSave').on('click', function() {
		...
		$('#projectReqstTable tbody').find('tr').each(function(index, item) {
			if($(this).find('input[name=reqCheckBox]').is(':checked')) {
				...
				let preemptiveQty = $(td).eq(projectReqstTable.column('preemptiveQty:name').index()).find('input').val();
		...
		$('#projectReqstTable tbody').find('tr').each(function(index, item) {
			if($(this).find('input[name=reqCheckBox]').is(':checked')) {
				...
				obj.preemptiveQty = data.preemptiveQty;
				...
				if(projectReqstTable.row(index_tr).data().designApprovalYn=='001') {
					...
					$(tr).find('td').each(function(index_td, td) {
						if($(td).find('input').length == 0 && $(td).find('select').length == 0) {
							...
							} else if(index_td == projectReqstTable.column('preemptiveQty:name').index()) {
								let value = $(td).text();
								$(td).html('<input type="text" style="min-width:100px;" onkeyup="numberFormat(this, \'int\')" class="form-control text-end" value="'+value+'">');
		...
		new Promise((resolve, reject) => {
			...
			setTimeout(function() {
				...
				$('#projectFileTable tbody').find('tr').each(function(index, item) {
					...
					let obj = new Object();
					obj.bizOrdProjectIdx = bizOrdProjectIdx;
					obj.idx = idx;
					obj.inputGubun = "001";
...
// 			},
			...
			{ data: 'itemCd', "className": "text-center align-middle", name : "itemCd",
				render: function(data, type, row, meta) {
					return '<input type="text" style="min-width:150px;" class="form-control" name="itemCd" value="'+itemGubun+'-" disabled>';
				}
			},
			{ data: 'itemGubun1', className : 'text-center align-middle', name : "itemGubun1",
				render: function(data, type, row, meta) {
					let html = '';
					html += '<select class="form-select" name="itemGubun1" onchange="smallGubunChange()" style="min-width:100px;">';
	...
	function cellFocusChange(table_id, tr_index, td_index) {
		let key = event.keyCode;
		let tdType = '';

		let tableRowCount = $('#'+table_id+' tbody tr').length;
		let tableColumnCount = $('#'+table_id+' tbody tr').eq(0).find('td').length;
		/*37: ←, 38: ↑, 39: →, 40: ↓ */
		if(key==37) {
			if(0==td_index) {
				td_index = tableColumnCount;
			} else {
				td_index--;
			}
			...
			if(tableColumnCount-1==td_index) {
				td_index = 0;
			} else {
				td_index++;
			}
		} else if(key==40){
			if(tableRowCount==tr_index) {
				tr_index = 0;
			} else {
				tr_index++;
			}
		...
		if(key == 37 || key == 39) {
			if(key==37) {
				for(let i =0; i>-tableColumnCount; i--) {
					if($('#'+table_id+' tbody tr').eq(tr_index).find('td').eq(td_index+i).find('input').val()!=undefined) {
						$('#'+table_id+' tbody tr').eq(tr_index).find('td').eq(td_index+i).find('input').focus();
						return false;
					} else if($('#'+table_id+' tbody tr').eq(tr_index).find('td').eq(td_index+i).find('select').val()!=undefined) {
						$('#'+table_id+' tbody tr').eq(tr_index).find('td').eq(td_index+i).find('select').focus();
						return false;
					}
				}
			} else {
				for(let i =0; i<tableColumnCount; i++) {
					let changeColumnIndex = 0;
					if(td_index+i>tableColumnCount) {
						changeColumnIndex = td_index + i - tableColumnCount - 1;
					} else {
						changeColumnIndex = td_index+i; 
					}
					
					if($('#'+table_id+' tbody tr').eq(tr_index).find('td').eq(changeColumnIndex).find('input').val()!=undefined) {
						$('#'+table_id+' tbody tr').eq(tr_index).find('td').eq(changeColumnIndex).find('input').focus();
						return false;
					} else if($('#'+table_id+' tbody tr').eq(tr_index).find('td').eq(changeColumnIndex).find('select').val()!=undefined) {
						$('#'+table_id+' tbody tr').eq(tr_index).find('td').eq(changeColumnIndex).find('select').focus();
						return false;
					}
				}
			}
		} else {
			if($('#'+table_id+' tbody tr').eq(tr_index).find('td').eq(td_index).find('input').val()!=undefined) {
				$('#'+table_id+' tbody tr').eq(tr_index).find('td').eq(td_index).find('input').focus();
			} else if($('#'+table_id+' tbody tr').eq(tr_index).find('td').eq(td_index).find('select').val()!=undefined) {
				$('#'+table_id+' tbody tr').eq(tr_index).find('td').eq(td_index).find('select').focus();
			}
		}
