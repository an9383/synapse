...
<script>
	...
	let workOrdProgressStatusTable = $('#workOrdProgressStatusTable').DataTable({
        ...
        ajax : {
			url : '<c:url value="/wm/workOrdPrcssStatusLstOrderByDealCorp"/>',
			type : 'POST',
			data : {
		...
		drawCallback: function() {
			...
			if(data.length != 0) {
				let dealCorpUniq = _.uniqBy(data, 'dealCorpIdx'); // 수주처 유니크 리스트
				for(var i = 0; i < dealCorpUniq.length; i++){
					//해당 수주처의 리스트
					let dealCorpList = _.filter(data, v => v.dealCorpIdx == dealCorpUniq[i].dealCorpIdx);
					let dealCorpLastIdx = _.maxBy(dealCorpList, (v => v.bizOrdDtlNo)).workOrdIdx;
					let dealCorpBizOrdQtySum = _.sumBy(dealCorpList, (v => isNaN(parseInt(v.bizOrdQty)) ? 0 : parseInt(v.bizOrdQty)));
					let dealCorpWorkOrdQtySum = _.sumBy(dealCorpList, (v => parseInt(v.workOrdQty)));
					let dealCorpFairQtySum = _.sumBy(dealCorpList, function(v){
																		if(v.workMethodJson != null && v.workMethodJson != ''){
																			let parseData = JSON.parse(v.workMethodJson);
																			let filterList = parseData.filter(v=>v.progressStatus == 'WC');
																			let finshPrcssCount = filterList.length;
																			if( finshPrcssCount > 0){
																				let fairQty = isEmptyCheck(filterList[ finshPrcssCount - 1 ].fairQty) ? 0 : filterList[ finshPrcssCount - 1 ].fairQty;
																				return parseInt(fairQty);
																			} else{
																				return 0;
																			}
																		} else {
					...
					let dealHtml = '';
					dealHtml += '<tr class="bg-success bg-gradient bg-opacity-10">';
					dealHtml += '	<td colspan="2" class="text-center">수주처합계</td>';
					dealHtml += '	<td colspan="1" class="text-center">'+dealCorpUniq[i].dealCorpNm+'</td>';
					dealHtml += '	<td colspan="1" class="text-end">'+dealCorpList.length+'</td>';
					dealHtml += '	<td colspan="1" class="text-end">'+addCommas(parseInt(dealCorpBizOrdQtySum))+'</td>';
					dealHtml += '	<td colspan="6" class="text-center"></td>';
					dealHtml += '	<td colspan="1" class="text-end">'+addCommas(parseInt(dealCorpWorkOrdQtySum))+'</td>';
					dealHtml += '	<td colspan="1" class="text-end">'+addCommas(parseInt(dealCorpFairQtySum))+'</td>';
					dealHtml += '	<td colspan="2" class="text-center"></td>';
					dealHtml += '</tr>';
					let dealNode = api.row('#'+dealCorpLastIdx).node();
					//중간노드
					$(dealNode).after(dealHtml);
				}
				//마지막노드
				let dealCorpLastList = _.filter(data, v => v.dealCorpIdx == dealCorpUniq[dealCorpUniq.length - 1].dealCorpIdx);
				let dealCorpLastIdx = _.maxBy(dealCorpLastList, (v => v.bizOrdDtlNo )).workOrdIdx;
				let dealCorpLastBizOrdQtySum = _.sumBy(data, (v => isNaN(parseInt(v.bizOrdQty)) ? 0 : parseInt(v.bizOrdQty)));
				let dealCorpLastWorkOrdQtySum = _.sumBy(data, (v => parseInt(v.workOrdQty)));
				let dealCorpLastFairQtySum = _.sumBy(data, function(v){
																	if(v.workMethodJson != null && v.workMethodJson != ''){
																		let parseData = JSON.parse(v.workMethodJson);
																		let filterList = parseData.filter(v=>v.progressStatus == 'WC');
																		let finshPrcssCount = filterList.length;
				...
				let dealLastHtml = '';
				dealLastHtml += '<tr class="bg-success bg-gradient bg-opacity-25">';
				dealLastHtml += '	<td colspan="3" class="text-center">총합계</td>';
				dealLastHtml += '	<td colspan="1" class="text-end">'+data.length+'</td>';
				dealLastHtml += '	<td colspan="1" class="text-end">'+addCommas(parseInt(dealCorpLastBizOrdQtySum))+'</td>';
				dealLastHtml += '	<td colspan="6" class="text-center"></td>';
				dealLastHtml += '	<td colspan="1" class="text-end">'+addCommas(parseInt(dealCorpLastWorkOrdQtySum))+'</td>';
				dealLastHtml += '	<td colspan="1" class="text-end">'+addCommas(parseInt(dealCorpLastFairQtySum))+'</td>';
				dealLastHtml += '	<td colspan="2" class="text-center"></td>';
				dealLastHtml += '</tr>';
				let dealLastNode = api.row('#'+dealCorpLastIdx).node();
				$(dealLastNode).next().after(dealLastHtml);
