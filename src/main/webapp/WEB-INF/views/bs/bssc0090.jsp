...
<script>
	...
	let excelTableArray = '';	//columns 값 지정 Array
	...
	let bizOrdOutProdTable = $('#bizOrdOutProdTable').DataTable({
		...
		buttons : [
			{ extend: 'excel',	text: 'Excel',	charset: 'UTF-8', bom: true ,
				filename : '출고집계표',//엑셀파일명 
				title : '출고집계표(레포트)',//엑셀파일안에 쓰일 제목
				...
				customizeData: function(data){
			        ...
			        let tableShowData = bizOrdOutProdTable.rows({page:'current'}).data().toArray();
			        data.body.slice(2); // 0 번째는 조회조건, 1번째는 헤더, 2번째부터 /여기에 실데이터 들어있음
				...
				/*exportOptions: {
                 	columns: excelTableArray	//원하는 값 지정
                },*/
			},
			/*{ extend: 'pdf',	text: 'PDF',	},
   ...
   //가짜 엑셀 버튼 클릭 시
	$('#btnPrint').on('click', function(){
		$('#my-spinner').show();
		setTimeout(function(){
			let table = document.getElementById("bizOrdOutProdTable");
			$(table).find('.filters').remove();
			// 새로운 tr 요소 생성
			let newRow = table.insertRow(0); // 맨 위에 추가하려면 인덱스 0으로 설정
			let newRow1 = table.insertRow(1); // 맨 위에 추가하려면 인덱스 0으로 설정
			let newRow2 = table.insertRow(2); // 맨 위에 추가하려면 인덱스 0으로 설정
			// td 요소 추가
			let newCell1 = newRow.insertCell(0);
			
			let newCell2 = newRow1.insertCell(0);
			
			let newRow2Cell1 = newRow2.insertCell(0);
			let newRow2Cell2 = newRow2.insertCell(1);
			let newRow2Cell3 = newRow2.insertCell(2);
			let newRow2Cell4 = newRow2.insertCell(3);
			let newRow2Cell5 = newRow2.insertCell(4);
			let newRow2Cell6 = newRow2.insertCell(5);
			let newRow2Cell7 = newRow2.insertCell(6);
			// td에 내용 추가
			newCell1.innerHTML = "출고집계표(레포트)";
			newRow2Cell1.innerHTML = "출고일자";
			newRow2Cell2.innerHTML = $('#startDate').val() + " ~ " + $('#endDate').val();
			newRow2Cell4.innerHTML = "제품코드";
			newRow2Cell5.innerHTML = $('#itemNm').val();
			newRow2Cell6.innerHTML = "거래처";
			newRow2Cell7.innerHTML = $('#searchDealCorpNm').val();
			
			
			// SheetJS 라이브러리 사용하여 Excel 파일로 변환
			//let workbook = XLSX.utils.table_to_book(table);
			
			// HTML 테이블을 SheetJS 라이브러리에서 인식 가능한 데이터 형식으로 변환
			let worksheet = XLSX.utils.table_to_sheet(table, {cellStyles:true});
			...
			worksheet["!cols"] = [{ width: 15 }, { width: 15 }, { width: 15 },
								  ...
								  { width: 15 }, { width: 15 }, { width: 15 },
								  { width: 15 }];
			//worksheet["!rows"] = [{ height: 30 }, { height: 20 }, { height: 20 }];
			...
			// 변환된 데이터를 Excel 파일에 추가
			let workbook = XLSX.utils.book_new();
			XLSX.utils.book_append_sheet(workbook, worksheet, "Sheet1");

			// Excel 파일 다운로드
			XLSX.writeFile(workbook, "출고집계표(레포트).xlsx");
			$('#my-spinner').hide();
		},100);
