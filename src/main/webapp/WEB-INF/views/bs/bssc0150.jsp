...
<script>
	...
	$('#btnOutReqSave').on('click', function(){
		...
		console.log(array)
		$.ajax({
			url: '<c:url value="/bs/outReqAdmIU"/>',
            type: 'POST',
            data: {
	...
	// 시리얼번호 목록조회
	$('#serialNoModalTable thead tr').clone(true).addClass('filters').appendTo('#serialNoModalTable thead'); // filter 생성
	let serialNoModalTable = $('#serialNoModalTable').DataTable({
		dom : "<'row'<'col-md-12 col-md-6'l><'col-md-12 col-md-6'f>>"
				+ "<'row'<'col-md-12'tr>>"
				+ "<'row pt-1'<'d-flex align-items-center d-flex'><'me-lg-auto'><'d-flex align-items-center justify-content-end'>>",
		language: lang_kor,
...
// 	});
	...
	$('#btnSerialNoModalSave').on('click', function(){
		if($('#serialNoModalTable').DataTable().row('.selected').length == 0){
			toastr.warning('수정할 행을 선택해주세요.');
			return false;
		}

		if($('#outReqAdmTable').DataTable().row('.selected').data().outQty < parseInt($('#serialNoModalTable tbody').find('.selected').length)+parseInt($('#outReqDtlTable').DataTable().data().length)){
		...
		let array = [];
		let state = true;
		$('#serialNoModalTable tbody').find('tr').each(function(index, item) {
			if($(this).hasClass('selected')){
				let node = $(item);
				let tr = $(node).parent().parent();
				let td = $(item).find('td');
				//Serial No
				let serialNo = $('#serialNoModalTable').DataTable().row($(this)).data().serialNo;
		...
		console.log(array)
		$.ajax({
			url: '<c:url value="/bs/outReqDtlIU"/>',
            type: 'POST',
            data: {
                'jsonArray'	:	JSON.stringify(array)
            },
	...
	$('#btnDeleteModalY').on('click', function() {
		...
		if(deleteType == 'ADM') {
			...
			if(outReqNo==null) {
				...
				$.ajax({
					url: '<c:url value="/bs/outReqAdmDel"/>',
		            type: 'POST',
		            data: {
			            'idx' 		:	outReqIdx,
		                'outReqNo'	:	outReqNo
			...
			if(idx=='') {
				...
				$.ajax({
					url: '<c:url value="/bs/outReqDtlDel"/>',
		            type: 'POST',
		            data: {
			            'idx' 		:	idx
		            },
